...
def generator(num_instances,min_demand, max_demand, min_costs_strade, max_costs_strade, min_costs_ferrovie, max_costs_ferrovie):
    ...
    for i in range(1, num_instances):
        ...
        min_cap_strade = gen_CapStrada((min_demand*10),Orders)
        print('min_cap_strade:'+ str(min_cap_strade))
        max_cap_strade = gen_CapStrada((max_demand/2),Orders)
        print('max_cap_strade:'+ str(max_cap_strade))

        min_cap_ferrovie = min_demand
        print('min_cap_ferrovie:' + str(min_cap_ferrovie))
        max_cap_ferrovie = min_cap_strade -1
        print('max_cap_ferrovie:' + str(max_cap_ferrovie))

        min_dist_ferrovie = gen_mDistFerrovie(n)
        print('min_dist_ferrovie:' +  str(min_dist_ferrovie))
