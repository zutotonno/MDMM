...
$dir = dir D:\Lavoro\GitHub\MDMM\dati\datibuoni\dati1NewLAGRANGE
foreach($d in $dir)
{
	...
	foreach($d2 in $dir2)
	{
		...
		$files_sol = Get-ChildItem $current_folder -Filter *.cmpl
		$files_xaml= Get-ChildItem $current_folder -Filter *.cmpl
		$sol_list = [System.Collections.ArrayList]@()
		$instance_list = [System.Collections.ArrayList]@()
		foreach ($f in $files){
			$curr_exec = python lagrange.py $f.FullName $f.FullName(rimuovere .cmpl e incollare .xaml)
